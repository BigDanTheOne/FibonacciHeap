...
public:
	...
	void ScaleUpArray() {
		...
		//std::vector<Key> new_arr(array_size * 2);
	...
	void ScaleDownArray() {
		...
		//std::vector<Key> new_arr(array_size * 0.7);
...
public:
	...
	Pointer insert(Key key) {
		if (size == 0) {
			...
			//Vector<Node<Key> *> new_buffer;
			buffer.arr = new_buffer;
			//delete new_buffer; _____crush___
		}
	...
	void IPriorityQueue(Iterator begin, Iterator end) {
		...
		int index = 0;
		// ALARM !!! ++++++++++++++++++++
		//std::vector<Node<Key> *> new_arr(curr_size);
		...
		// TODO: REMOVE THIS 
		//delete[] new_arr; _______CRUSH______
		//delete new_arr;_______CRUSH______
